...
from backend.models import User, Session, Recipe, WorkoutTemplate, MealPlan, GroceryList, Exercise, SetEntry, Ingredient
...
async def test_seed_database():
    ...
    db_url = os.getenv("DATABASE_URL", "mongodb://localhost:27017/ironlog_seed")
    ...
    try:
        ...
        # Verification
        assert await User.find_one(User.username == "admin_boss") is not None
        assert await User.find_one(User.username == "fitness_fan") is not None
        assert await Recipe.find_one(Recipe.name == "Post-Workout Power Shake") is not None
        assert await Session.find_one(Session.name == "Powerlifting A") is not None
