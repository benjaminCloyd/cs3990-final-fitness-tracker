...
def require_admin(user: TokenData):
    """Enforce admin-only access for administrative operations."""
    if user.role != "admin":
        raise HTTPException(status_code=403, detail="Admin access required.")


async def get_recipe_or_404(recipe_id: str, user: TokenData) -> Recipe:
    """Retrieve a recipe by ID or raise 404; enforce ownership or admin bypass."""
    try:
...
async def list_recipes(user: TokenData = Depends(authenticate)):
    """Retrieve all recipes for the authenticated user (or all for admins)."""
    if user.role == "admin":
...
async def delete_recipe(recipe_id: str, user: TokenData = Depends(authenticate)):
    """Permanently delete a recipe from the library."""
    recipe = await get_recipe_or_404(recipe_id, user)
    await recipe.delete()
...
async def get_meal_plan_macros(plan_id: str, user: TokenData = Depends(authenticate)):
    """Aggregate total daily macro consumption based on the assigned recipe slots."""
    try:
...
async def generate_grocery_list(plan_id: str, user: TokenData = Depends(authenticate)):
    """Consolidate all ingredients from all recipes within a weekly plan."""
    try:
        oid = PydanticObjectId(plan_id)
    except Exception:
        raise HTTPException(status_code=400, detail="Invalid plan ID.")
...
async def download_grocery_list(list_id: str, user: TokenData = Depends(authenticate)):
    """Generate and return a text-formatted grocery list, deleting it after transmission."""
    try:
...
async def list_all_files(user: TokenData = Depends(authenticate)):
    """List all uploaded recipe images in the server storage (Admin only)."""
    require_admin(user)
    files = []
...
async def delete_file(filename: str, user: TokenData = Depends(authenticate)):
    """Permanently delete an uploaded image file from the server storage (Admin only)."""
    require_admin(user)
